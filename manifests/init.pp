...
class auditd (
  ...
  Enum['RAW', 'NOLOG'] $log_format,
  String $log_group,
  Integer $priority_boost,
  Enum['none', 'incremental', 'data', 'sync'] $flush,
  Integer $freq,
  Integer $num_logs,
  Enum['lossy', 'lossless'] $disp_qos,
  Stdlib::Absolutepath $dispatcher,
  Enum['none', 'hostname', 'fqd', 'numeric', 'user'] $name_format,
  String $admin,
  Integer $max_log_file,
  Enum['ignore', 'syslog', 'suspend', 'rotate', 'keep_logs'] $max_log_file_action,
  Integer $space_left,
  Enum['ignore', 'syslog', 'email', 'exec', 'suspend', 'single', 'halt'] $space_left_action,
  String $action_mail_acct,
  Integer $admin_space_left,
  Enum['ignore', 'syslog', 'email', 'exec', 'suspend', 'single', 'halt'] $admin_space_left_action,
  Enum['ignore', 'syslog', 'exec', 'suspend', 'single', 'halt'] $disk_full_action,
  Enum['ignore', 'syslog', 'exec', 'suspend', 'single', 'halt'] $disk_error_action,
  Variant[Undef, Integer] $tcp_listen_port,
  Integer $tcp_listen_queue,
  Variant[Undef, Integer] $tcp_max_per_addr,
  Variant[Undef, String] $tcp_client_ports,
  Integer $tcp_client_max_idle,
  Enum['yes', 'no'] $enable_krb5,
  String $krb5_principal,
  Variant[Undef, Stdlib::Absolutepath] $krb5_key_file,

  Stdlib::Absolutepath $rules_file,
  Boolean $manage_audit_files,
  Integer $buffer_size,

  Integer $audisp_q_depth,
  Enum['ignore', 'syslog', 'suspend', 'single', 'halt'] $audisp_overflow_action,
  Integer $audisp_priority_boost,
  Integer $audisp_max_restarts,
  Variant[Undef, String] $audisp_name,
  ...
  file { '/etc/audit/auditd.conf':
    ...
    group   => 'root',
    mode    => '0640',
    content => epp('auditd/auditd.conf.erb'),
  }
  ...
  concat::fragment{ 'auditd_rules_begin':
    target  => $rules_file,
    content => epp('auditd/audit.rules.begin.fragment.erb'),
    order   => 0
  }
  file { '/etc/audisp/audispd.conf':
    ensure  => 'file',
    ...
    group   => 'root',
    mode    => '0640',
    content => epp('auditd/audispd.conf.erb'),
  }
